...
class NewsQueries:
    ...
    def bulk_save_articles(articles_data):
        ...
        try:
            ...
            # Get all existing article_ids to avoid duplicates
            existing_article_ids = set(
                article_id[0] for article_id in 
                db.session.query(NewsArticle.article_id).filter(
                    NewsArticle.article_id.in_([article['article_id'] for article in articles_data])
                ).all()
            )
            
            for article_data in articles_data:
                # Check if article already exists using the set for faster lookup
                if article_data['article_id'] not in existing_article_ids:
                    try:
                        article = NewsArticle.from_api_data(article_data)
                        db.session.add(article)
                        saved_articles.append(article)
                    except Exception as e:
                        # Log individual article errors but continue with others
                        print(f"Error processing article {article_data.get('article_id', 'unknown')}: {e}")
                        continue
